                    ...
                    collection.find({_id: mtid}).limit(1).toArray(function (err, ret) {
                        ...
                        me.converter.toIDs(['_revenue', '_firstcampaign', '_lastcampaign', '_campaign', '_ctime', '_mtid', '_reftype',
                            ...
                            if (typeid.toLowerCase() === 'purchase') {
                                ...
                                if (user[ids._listProduct] == null || !Array.isArray(user[ids._listProduct])) {
                                    user[ids._listProduct] = [];
                                }
                                if (user[ids._listProduct].length > 4) {
                                    user[ids._listProduct].shift();
                                }
                                datax[ids._listProduct] = [];
                                user[ids._listProduct].push(data.pname);
                                datax[ids._listProduct] = user[ids._listProduct];
                            ...
                            // Add last active
                            datax[ids._lastSeen] = Math.round(new Date().getTime() / 1000);
